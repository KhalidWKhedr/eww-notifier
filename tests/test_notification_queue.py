...
def test_add_and_get_notification(temp_notification_file):
    queue = NotificationQueue()
    notif = {'notification_id': '1', 'summary': 'Test', 'body': 'Body', 'expire_timeout': 10000,
             'timestamp': time.time()}
...
def test_remove_notification(temp_notification_file):
    queue = NotificationQueue()
    notif = {'notification_id': '2', 'summary': 'Test2', 'body': 'Body2', 'expire_timeout': 10000,
             'timestamp': time.time()}
    queue.add_notification(notif)
...
def test_clear_notifications(temp_notification_file):
    queue = NotificationQueue()
    notif = {'notification_id': '3', 'summary': 'Test3', 'body': 'Body3', 'expire_timeout': 10000,
             'timestamp': time.time()}
    queue.add_notification(notif)
    queue.clear()
...
def test_expired_notification_cleanup(temp_notification_file):
    queue = NotificationQueue()
    old_notif = {'notification_id': '4', 'summary': 'Old', 'body': 'Old', 'expire_timeout': 1,
